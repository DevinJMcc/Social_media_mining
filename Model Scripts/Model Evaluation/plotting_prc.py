...
def runDecisionTreeClassifier(x_train, y_train, x_test, y_test, p):
    ...
    average_precision = average_precision_score(y_test, etc_predictions)

    print('Average precision-recall score: {0:0.4f}'.format(
        average_precision))

    fig = plt.figure()
    disp = plot_precision_recall_curve(clf, x_test, y_test, ax = plt.gca(), name = "dt")
...
def runGradientBoostingClassifier(x_train, y_train, x_test, y_test, p):
    ...
    disp = plot_precision_recall_curve(clf, x_test, y_test, ax = plt.gca(), name = "gb")
...
def runAdaBoostClassifier(x_train, y_train, x_test, y_test, p):
    ...
    disp = plot_precision_recall_curve(clf, x_test, y_test, ax = plt.gca(), name = "ab")
...
def runExtraTreesClassifier(x_train, y_train, x_test, y_test, p):
    ...
    disp = plot_precision_recall_curve(clf, x_test, y_test, ax = plt.gca(), name = "et")
...
def runSupportVec(x_train, y_train, x_test, y_test, p):
    ...
    disp = plot_precision_recall_curve(clf, x_test, y_test, ax = plt.gca(), name = "svm")
...
def runXGBoost(x_train, y_train, x_test, y_test, p):
    ...
    disp = plot_precision_recall_curve(clf, x_test, y_test, ax = plt.gca(), name = "xgb")
    #plt.ylim([.5, 1.0])
    #plt.gca().legend(
...
def random_trees(x_train, y_train, x_test, y_test, p):
    ...
    disp = plot_precision_recall_curve(clf, x_test, y_test, ax = plt.gca(), name = "rf")
    # disp.ax_.set_title('2-class Precision-Recall curve')
    plt.ylim([.5, 1.0])
    plt.gca().legend(
           loc="top right",   # Position of legend
...
def nueral_net(x_train, y_train, x_test, y_test, p):
    ...
    disp = plot_precision_recall_curve(clf, x_test, y_test, ax = plt.gca(), name = "MLP")
    plt.ylim([.5, 1.0])
    plt.gca().legend(
           loc="top right",   # Position of legend
           )
...
if __name__ == '__main__':
    ...
    dd = data.split('/')[-1].split('.')[0]
    fname = 'prc_%s.pdf'%(dd)
    for alg in [0,1,3,5,7]:
